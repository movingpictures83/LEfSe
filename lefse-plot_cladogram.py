...
def read_params(args):
	...
	parser.add_argument('--label_font_size',dest="label_font_size", type=str, default="4")
	...
	parser.add_argument('--dpi',dest="dpi", type=int, default=720)
...
def read_data(input_file,params):
	...
	abundances = []
	for element in rows:
		abundances.append(float(element[1]))
...
def uniqueid():
	for l in 'abcdefghijklmnopqrstuvwxyz': yield l
...
def draw_tree(out_file,tree,params):
	...
	while totseps > np.pi:
		...
		totseps = sum([s*nlev[i] for i,s in enumerate(seps[:-1])])
	if clade_sep_err: print('clade_sep parameter too large, lowered to'+str(params['clade_sep']))
