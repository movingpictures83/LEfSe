...
def sort_by_cl(data,n,c,s,u):
    ...
    if n == 3:
        data.sort(key=cmp_to_key(sort_lines3))
...
def get_class_slices(data):
    ...
    #exit(1)
    #print(list(data[0]))
    #previous_class = data[0]
    #previous_subclass = data[1]
    previous_class = data[0][0]
    previous_subclass = data[1][0]
    subclass_slices = []
    class_slices = []
    last_cl = 0
    last_subcl = 0
...
if  __name__ == '__main__':
    ...
    first_line = modify_feature_names(first_line)
    #print(first_line)
    #exit(1)
    #print(sort_by_cl(data[1:], ncl, params['class']-1,
    ...
    #          params['subclass']-1 if not params['subclass'] is None else None,
    #          params['subject']-1 if not params['subject'] is None else None))
    data = [first_line] + sort_by_cl(data[1:], ncl, params['class']-1,
              params['subclass']-1 if not params['subclass'] is None else None,
              params['subject']-1 if not params['subject'] is None else None)
    #data = list(data)
    #print(data)
...
#   data = remove_missing(data,params['missing_p'])
    ...
    #print("AFTER SORT")
    #print(cls_i)
    for i in range(len(cls_i)):
       cls_i[i] = (cls_i[i][1], cls_i[i][0])
    ...
    #cls_i.sort(key=cmp_to_key(lambda x, y: -cmp(x[1],y[1])))
    #cls_i.sort(key=(lambda x: -cmp(x[1])))
    #print(cls_i)
    #print(data)
    #print("POPPING NOW")
    #exit(1)
...
#   if 'subclass' in cls.keys(): cls = group_small_subclasses(cls,params['subcl_min_card'])
    ...
    #print(list(cls.values()))
    #exit(1)
    #class_sl,subclass_sl,class_hierarchy = get_class_slices(list(cls.values()))
    class_sl,subclass_sl,class_hierarchy = get_class_slices([cls['class']] + [cls['subclass']] + [cls['subject']])
    #print(class_sl)
    #print(subclass_sl)
    ...
    for j in range(1, len(data)):
       for i in range(len(data[j])):
          feats[data[0][i]].append(data[j][i])
