...
if __name__ == '__main__':
    ...
    for feat_name, feat_values in feats.items():
       items.append((feat_name, feat_values))
    for feat_name,feat_values in items:#feats.items():
        if params['verbose']:
    ...
    if len(feats) > 0:
        print("Number of significantly discriminative features:"+str(len(feats))+"("+str(kw_n_ok)+") before internal wilcoxon")
    ...
    print("Number of discriminative features with abs LDA score >"+str(params['lda_abs_th'])+":"+str(len(lda_res_th)))
